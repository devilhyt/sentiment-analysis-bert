...
def write_file(mode, data):
    with open('./data/' + board + ".csv", mode, newline='',encoding="UTF-8") as csvfile:
        if data:
            writer = csv.writer(csvfile)
            writer.writerow([data, board])

def create_file():
    with open('./data/' + board + '.csv', 'w', newline='',encoding='UTF-8') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['content', 'board'])
